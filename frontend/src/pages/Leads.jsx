            ...
            <div className="flex items-center space-x-2 overflow-x-auto pb-2">
                {['All', 'pending', 'contacted', 'closed'].map((f) => (
                    <button
                        key={f}
                        onClick={() => setFilter(f)}
                        className={`w-fit border border-black/20 relative p-2 px-4 focus:ring-0 text-sm font-bold text-black/70  ${filter === f ? 'bg-indigo-600' : 'bg-white text-gray-500 border border-black/30 hover:bg-gray-50'
                            }`}
                    >
                        <img src="/public/bg-img.png" className='absolute w-full h-full object-cover opacity-20 inset-0' alt="" />
                        {f}
