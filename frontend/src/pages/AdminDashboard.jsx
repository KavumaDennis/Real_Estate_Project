        ...
        <div className="space-y-10">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="px-4 sm:px-6 py-2 sm:py-3 border border-black/10 bg-amber-600 text-xs text-start font-black uppercase tracking-widest text-white hover:bg-blue-600 transition shadow-lg">Monitoring real-time performance and system health.</p>
                </div>
                <div className="flex items-center space-x-2 px-4 sm:px-6 py-2 sm:py-3 border border-black/10 bg-teal-700 text-xs text-start font-black uppercase tracking-widest text-white hover:bg-blue-600 transition shadow-lg cursor-pointer">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                {statCards.map((card) => (
                    <div key={card.name} className="bg-teal-700 p-4 border border-black/30 shadow-sm hover:shadow-xl transition-all duration-300 group">
                        <div className="flex items-center justify-between mb-6">
                            <div className={`p-4 bg-amber-600 text-white transition-transform duration-300`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-10">
                ...
                <div className="lg:col-span-2 bg-white/50 h-fit p-10 border border-dashed border-black/30 shadow-sm">
              ...
              <div className="bg-amber-600 p-8 border border-black/20 shadow-sm">
                        <div className="flex items-center justify-between mb-8">
                            <h3 className="text-lg font-black text-white">New Agents</h3>
                            <Link to="/admin/users" className="text-white p-2 bg-teal-700 transition">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10">
                <div className="bg-white/50 p-8 border border-dashed border-black/30 shadow-sm overflow-hidden">
                            ...
                            <div key={prop.id} className="py-4 px-2 hover:bg-gray-50/50 transition flex items-center justify-between group">
                                <div className="flex items-center space-x-4 overflow-hidden">
                                    ...
                                    <div className="overflow-hidden">
                                        ...
                                        <p className="text-[10px] text-start text-teal-700 font-bold uppercase tracking-widest mt-0.5">{prop.location?.name}</p>
                                    </div>
                                </div>
                                <span className="text-sm font-black text-amber-600 group-hover:translate-x-1 transition-transform animate-in fade-in duration-500">
                        ...
                        <div className="space-y-4">
                            {stats.recent_activity?.map((activity, i) => (
                                <div key={i} className="flex items-center space-x-4 p-3 bg-white/5 border border-white/10 hover:bg-white/10 transition group/item">
                                    ...
                                    <div className="overflow-hidden">
                                        <p className="text-sm text-start text-amber-500 font-black truncate">{activity.title}</p>
                                        <p className="text-[10px] text-start text-white font-medium truncate">{activity.desc}</p>
                                    </div>
                                </div>
                            ))}
