                    ...
                    <table className="w-full text-left border-collapse relative z-10">
                        ...
                        <thead>
                            <tr className="bg-green-600 border-b border-gray-100">
                                ...
                                <th className="px-8 py-4 text-[10px] font-black text-indigo-100 uppercase tracking-[0.2em]">Target Property</th>
                                <th className="px-8 py-4 text-[10px] font-black text-indigo-100 uppercase tracking-[0.2em]">Rating</th>
                                <th className="px-8 py-4 text-[10px] font-black text-indigo-100 uppercase tracking-[0.2em]">Status</th>
                                <th className="px-8 py-4 text-[10px] font-black text-indigo-100 uppercase tracking-[0.2em] text-right">Actions</th>
                            </tr>
                        </thead>
                            ...
                            {filteredReviews.map((r) => (
                                <tr key={r.id} className="hover:bg-gray-50/30 transition-colors group">
                                    <td className="px-8 py-6">
                                        <div>
                                            <p className="font-bold text-black text-sm line-clamp-2 mb-2 italic">"{r.comment}"</p>
                                            <div className="flex items-center space-x-2">
                                                <div className="h-6 w-6 bg-blue-100 border border-black/30 rounded-full flex items-center justify-center text-[10px] font-black text-blue-600">
                                                    {r.user?.name.charAt(0)}
                                                </div>
                                                <p className="text-xs font-black text-green-600 uppercase tracking-widest">{r.user?.name}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className="flex items-center text-sm font-bold text-gray-600">
                                            {r.property ? (
                                                <>
                                                    <HiOutlineHome className="mr-2 text-indigo-600" />
                                                    <span className="truncate max-w-[150px] text-indigo-600">{r.property.title}</span>
                                                </>
                                            ) : (
                                                <>
                                    ...
                                    <td className="px-8 py-6">
                                        <span className={`px-4 py-1.5 text-[10px] font-black uppercase tracking-widest inline-flex items-center border border-black/30 ${r.is_approved ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                                    ...
                                    <td className="px-8 py-6">
                                        <div className="flex justify-end space-x-2">
                                            <button
                                                ...
                                                className={`p-2 px-2.5 border border-black/30 transition shadow-sm ${r.is_approved ? 'bg-red-500' : 'bg-green-600'}`}
                                            ...
                                            <button
                                                ...
                                                className="p-2 px-2.5 border border-black/30 text-indigo-100 bg-red-500 transition shadow-sm"
                                            >
