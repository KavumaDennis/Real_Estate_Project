        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="px-4 sm:px-6 py-2 sm:py-3 border border-black/10 bg-amber-600 text-xs text-start font-black uppercase tracking-widest text-white hover:bg-blue-600 transition shadow-lg shrink-0">Manage ratings and feedback.</p>
                </div>

                <div className="flex items-center bg-white/60 px-4 py-2 border border-black/30 w-fit">
                    <div>
                        <p className="text-xl text-start font-black text-amber-600 leading-none">{averageRating}</p>
                ...
                <div className="flex items-center space-x-2 overflow-x-auto pb-1 scrollbar-hide">
                    {['all', 'approved', 'pending'].map((f) => (
                        <button
                            key={f}
                            onClick={() => setFilter(f)}
                            className={`min-w-fit px-6 py-2 border border-black/20 focus:ring-0 text-xs font-black uppercase tracking-widest transition flex-shrink-0 ${filter === f ? 'bg-teal-700 text-white' : 'bg-white text-black hover:bg-gray-50'
                                }`}
                        >
                            {f}
                        </button>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 gap-6">
                    {filteredReviews.map((review) => (
                        <div key={review.id} className="bg-white/50 p-6 sm:p-8 border border-dashed border-black/30 shadow-sm hover:shadow-md transition">
                            <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
                                <div className="flex-grow">
                                    ...
                                    <div className="relative text-teal-700 pl-8">
                                        <HiAnnotation className="absolute left-0 top-0 h-6 w-6 text-teal-700/30 -scale-x-100" />
                                ...
                                <div className="flex sm:flex-row lg:flex-col items-center justify-end gap-2 lg:border-l lg:border-black/5 lg:pl-6">
                                    <button
